...
const PLAYER_SHAPES = ['circle', 'triangle', 'square', 'rectangle', 'rhombus', 'trapezoid', 'pentagon', 'star'];
const MAX_LAPS = 24;
...
const ITEM_DATA = [
  ...
  },
  {
    ...
    title: '<大麻>',
    price: 500,
    desc: '麻醉自己，短暫逃避現實，使用後壓力 -100（最低0），體力+40，精神+50，信念-10。',
  ...
  },
  {
    ...
    title: '<奉獻>',
    price: 500,
    desc: '使用後信念 +20（最高100），精神+20，壓力-30，向宗教組織奉獻後心靈踏實。',
  ...
  },
{
  ...
  title: '<報串>',
  price: 3000,
  desc: '全面威脅舉報所有人：其他所有玩家財力 -500，特定玩家則-1500(最多減至0)，然後把減少的部份全部轉移到自己身上。',
...
const ShapeSVG = ({ color, shape, size = 16, className = "" }) => {
  ...
  switch(shape) {
    ...
    case 'star': content = <polygon points="50,5 63,38 98,38 70,59 81,95 50,75 19,95 30,59 2,38 37,38" fill={color} stroke={stroke} strokeWidth={strokeW}/>; break;
...
const canStillPursueGoalForAI = useCallback(
  (p, goalId) => {
    if (p.victoryTitle) return false;
...
const countFirstLayerGoalsStillViable = useCallback(
  (player, allPlayers, simulatedPlayerPatch = {}) => {
    const plan = player.goalPlan;
    if (!plan || plan.length === 0) return 0;

    const firstLayer = plan[0]; // 第 1 層：山大王 + 打工/Leisure + 卷王/蛇王
    if (!firstLayer || firstLayer.length === 0) return 0;

    // 用 patch 模擬執行某個行動後的玩家狀態
    const tempPlayer = { ...player, ...simulatedPlayerPatch };

    let count = 0;
    firstLayer.forEach(goalId => {
      if (canStillPursueGoalForAI(tempPlayer, goalId)) {
        count++;
      }
    });
...
const handleRecoveryAndEvent = useCallback(
  (playerIndex, recoveryTs) => {
    setPlayers(prev => {
      ...
      setTimeout(() => {
        // ★ Strict Mode 去重：同一個 pendingRecovery ts 只允許抽一次卡
        if (lastDrawTsRef.current === recoveryTs) {
          return;
        }
        lastDrawTsRef.current = recoveryTs;
...
const runRecoveryOnce = useCallback(
  (recovery) => {
    const { playerIndex, ts: recoveryTs } = recovery;

    if (hasProcessedRecoveryThisTurn) {
      return;
    }
  ...
  return;
    ...
    if (itemToUse.id === "weed") {
      setPlayers(prev => {
        const next = [...prev];
        const p = { ...next[playerIdx] };
        p.stamina = clamp(p.stamina + 40, 0, 100);
        p.stress = clamp(p.stress - 100, 0, 100);
        p.spirit = clamp(p.spirit + 50, 0, 100);
        p.belief = clamp(p.belief - 10, 0, 100);
        p.hasUsedWeed = true;
...
if (itemToUse.id === "donation") {
  ...
  return;
    ...
    if (itemToUse.id === "reportall") {
      ...
      addLog(
        `📢 ${player.name} 使用「報串」：其他所有玩家財力-500，特定玩家則-1500(最多減至0)，並把減少的部份轉移到自己身上！`
      );
      setShowItemEffect(itemToUse);
      setShowInventory(null);
      setTimeout(
...
const triggerSocialSubsidy = useCallback(
  (reason = '財力不足') => {
    ...
    const willUnlockMadKing =
      ...
      hasUsedWeed &&
      subsidyCountBefore + 1 >= 3;

    // ✅ 2. 如果確定會解鎖，先馬上彈出隱藏目標 modal
    if (willUnlockMadKing) {
      setHiddenGoalPopup({
    ...
    setPlayers(prev => {
      ...
      p.socialSubsidyCount = subsidyCountBefore + 1;
  ...
  if (tempPlayer.blessingBonus) {
    ...
    tempPlayer.blessingExpireLap = tempPlayer.lap;
      ...
      // 同步整個 tempPlayer 回 state（只覆蓋移動相關欄位，保留 items 等）
      setPlayers((prev) => {
        const next = [...prev];
        const prevP = next[currentTurnIndex];

        next[currentTurnIndex] = {
          ...prevP,
          pos: tempPlayer.pos,
          lap: tempPlayer.lap,
          stamina: tempPlayer.stamina,
          stress: tempPlayer.stress,
          spirit: tempPlayer.spirit,
          wealth: tempPlayer.wealth,
          isFinished: tempPlayer.isFinished,
          hasLandedOnWork: tempPlayer.hasLandedOnWork,
          hasLandedOnHoliday: tempPlayer.hasLandedOnHoliday,
          lastPlannedStopCellIndex: tempPlayer.lastPlannedStopCellIndex,
          skipCellResolveOnce: tempPlayer.skipCellResolveOnce,
          longInvestmentBonus: tempPlayer.longInvestmentBonus,
          blessingBonus: tempPlayer.blessingBonus,
          blessingExpireLap: tempPlayer.blessingExpireLap,
          belief: tempPlayer.belief,
        };
  ...
  // ① 先從 snapshot 讀取當前玩家與事件，計算邪教上帝是否會解鎖
  ...
  const beliefBefore = currentPlayer.belief || 0;
  const evId = event.event?.id || event.id;
  const isReligionEvent = evId === 'w21' || evId === 'h21';
  const hasUsedDonation = !!currentPlayer.hasUsedDonation;

  // 邪教上帝解鎖條件：尚未解鎖、是宗教宣傳、事件前信念為 0，且曾經用過奉獻
  const willUnlockCultGod =
    !currentPlayer.hasUnlockedCultGod &&
    isReligionEvent &&
    beliefBefore === 0 &&
    hasUsedDonation;

  // ② 若這次事件會解鎖邪教上帝，先立刻彈出提示 modal
  if (willUnlockCultGod) {
    setHiddenGoalPopup({
      playerName: currentPlayer.name,
      goalTitle: '邪教上帝',
      message:
  ...
  setPlayers(prev => {
    ...
    // --------- 邪教上帝解鎖（改用 willUnlockCultGod） ---------
    if (willUnlockCultGod) {
      p.hasUnlockedCultGod = true;
      p.hiddenGoals = [...(p.hiddenGoals || []), '邪教上帝'];

      addLog(
        `✨ ${p.name} 觸發隱藏目標【邪教上帝】：在信念為 0 且曾使用過「奉獻」的情況下接受宗教宣傳，若之後使用「奉獻」達 10 次且信念達 100，即可成就邪教上帝！`
      );
    }
...
const AI_ITEM_DATA = useMemo(
  () =>
    ITEM_DATA.map((it) => ({
      id: it.id,
      price: it.price,
      // 順便附帶 title 讓 AI log 也能用（雖然真正購買會再用 ITEM_DATA 找一次）
      title: it.title,
    })),
  []
);
...
const buildGoalPlanForAI = useCallback(
  (player) => {
    ...
    const combos = [
      {
        id: 1,
        goals: [
          AI_GOALS.KING_OF_COMPANY,   // 山大王
          AI_GOALS.KING_OF_WORK,      // 打工皇帝
          AI_GOALS.KING_OF_COMPETITION, // 卷王
          AI_GOALS.BAD_LUCK_KING,     // 地獄黑仔王
        ],
      },
      {
        id: 2,
      ...
      },
      {
        ...
        goals: [
          AI_GOALS.KING_OF_COMPANY,   // 山大王
          AI_GOALS.KING_OF_LEISURE,   // King of Leisure
        ],
      },
    ];
    ...
    // 在組合內隨機選主目標（之後可以再按狀況重排）
    const goalsCopy = [...pickedCombo.goals];
    const primaryIndex = Math.floor(Math.random() * goalsCopy.length);
    const primary = goalsCopy[primaryIndex];

    const secondary = goalsCopy.filter(g => g !== primary);

    // 初始時還沒有「瘋王／邪教上帝」的高階目標，
    // 但之後一旦解鎖，就會把 primary 切換成它們。
    return {
      comboId: pickedCombo.id,
      primary,       // 目前主目標
      secondary,     // 同一組合內的副目標清單
      // 之後可加：highPriority: null | '瘋王' | '邪教上帝'
    };
  },
  [AI_GOALS]
);

// ★ 新版：從 goalPlan 中選擇本回合要追的目標
// - 若已解鎖瘋王／邪教上帝，優先選它們中仍可追的那一個
// - 否則以 goalPlan.primary 為主目標，若不可行則在 secondary 中找替代
const pickNextGoalFromPlan = useCallback(
  (player, allPlayers) => {
    const plan = player.goalPlan;
    if (!plan) return null;

    const progressList = evaluateGoalProgress(player, allPlayers);
    const getScore = (goalId) => {
      const found = progressList.find(g => g.goal === goalId);
    ...
    if (player.hasUnlockedMadKing) {
      highPriorityGoals.push(AI_GOALS.MAD_KING);
    }
    if (player.hasUnlockedCultGod) {
      highPriorityGoals.push(AI_GOALS.CULT_GOD);
    }

    const viableHigh = highPriorityGoals.filter(goalId =>
      canStillPursueGoalForAI(player, goalId)
    );

    if (viableHigh.length > 0) {
    ...
    if (viableSecondary.length === 0) {
      return null;
    }

    if (viableSecondary.length === 1) {
    ...
    return sortedSecondary[0];
  },
  [evaluateGoalProgress, canStillPursueGoalForAI, AI_GOALS]
);
...
const evaluateBuyValue = useCallback(
  (ai, aiGoal) => {
    ...
    if (wealth < 1200 && stamina >= 40) return 0;
  ...
  (ai, idx, aiGoal) => {
    ...
    items.forEach((it, itemIdx) => {
      ...
      if (score === -9999) {
        ...
        if (aiGoal === AI_GOALS.CULT_GOD && ai.hasUnlockedCultGod) {
          s += 60;
        }
        ...
        let bestTargetWealth = 0;

        players.forEach((p, pIdx) => {
          if (!p || pIdx === idx || p.victoryTitle) return;

          const otherShares = (p.items || []).filter(x => x.id === 'companyshare').length;
          const diff = otherShares - myShares;

          if (diff > bestTargetShareDiff) {
            bestTargetShareDiff = diff;
          }

          if (otherShares === 0 && p.wealth > bestTargetWealth) {
            bestTargetWealth = p.wealth;
          }
        });

        if (bestTargetShareDiff >= 1) {
          s += 80 * bestTargetShareDiff;
        }

        if (bestTargetShareDiff <= 0 && bestTargetWealth >= 1000) {
          s += 40 + bestTargetWealth * 0.01;
        }
        ...
        if (ai.blessingBonus && (ai.blessingExpireLap || 0) > (ai.lap || 0)) {
          s *= 0.4;
        }
      ...
      if (score > 0 && (!best || score > best.score)) {
        best = { type: 'USEITEM', itemIdx, score, itemId: it.id };
      }
    });
...
const runAI = useCallback(
  (idx) => {
    ...
    if (!aiGoal || !canStillPursueGoalForAI(ai, aiGoal)) {
      const nextGoal = pickNextGoalFromPlan(ai, players);
      aiGoal = nextGoal || AI_GOALS.KING_OF_WORK;

      setPlayers(prev => {
        const next = [...prev];
        const p = { ...next[idx] };
        p.currentGoal = aiGoal;
        next[idx] = p;
        return next;
      });
    ...
    const beforeCount = countFirstLayerGoalsStillViable(updated, players, {});
    ...
    for (const s of candidateSteps) {
      ...
      if (isCombo3 && leisureStillViable && finalCell.type === 'work') {
        // combo3：King of Leisure 仍可行 → 不踩任何工作格
        continue;
      }
      ...
      if (finalCell.type === 'work') {
        ...
        } else if (finalCell.name === '工作日3') {
          simulatedPatch.work3Count = (updated.work3Count || 0) + 1;
        } else if (finalCell.name === '工作日4') {
          simulatedPatch.work4Count = (updated.work4Count || 0) + 1;
        } else if (finalCell.name === '工作日5') {
          simulatedPatch.work5Count = (updated.work5Count || 0) + 1;
        }
      }

      if (finalCell.holiday) {
        simulatedPatch.hasLandedOnHoliday = true;
      }

      const afterCount = countFirstLayerGoalsStillViable(
        updated,
        players,
        simulatedPatch
      );
      ...
      if (delta < 0) {
        ...
        const factor = beforeCount || 1;
        score += delta * basePenalty * factor;
      }
...
// 1) 遊戲開始時，為 AI 初始化 goalPlan / currentGoal
useEffect(() => {
  ...
  setPlayers(prev =>
    prev.map(p => {
      ...
      const plan = buildGoalPlanForAI(p);
      const initialGoal = plan[0]?.[0] || null;
...
}, [gameState, buildGoalPlanForAI, setPlayers]);
...
useEffect(() => {
  ...
  runRecoveryOnce(pendingRecovery);

  // 回合實際切換仍由 applyEventEffect / 事件 modal 裡的 passTurn 處理（保持原設計）
  setPendingRecovery(null);
}, [pendingRecovery, runRecoveryOnce]);
